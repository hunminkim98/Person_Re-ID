...
class RTMDet(BaseTool):
    ...
    def __call__(self, image: np.ndarray):
        ...
        # print(f"RTMDet results: {results}")
        return results
    ...
    def postprocess(
        ...
        if outputs.shape[-1] == 4:
            ...
            if dets is not None:
                dets[:, :4] /= ratio  # 박스 좌표를 원본 이미지 크기에 맞게 조정
                return dets  # [x1, y1, x2, y2, score, class_id] 형식으로 반환
            else:
                return np.array([])
            ...
            final_boxes /= ratio
            return np.column_stack((final_boxes, final_scores))  # [x1, y1, x2, y2, score] 형식으로 반환
